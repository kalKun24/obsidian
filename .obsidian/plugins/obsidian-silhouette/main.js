...
var require_datetime = __commonJS({
  "node_modules/owlelia/dist/bundle/datetime.js"(exports) {
    ...
    var DateTime10 = class _DateTime extends vo_1.ValueObject {
      /**
...
// src/domain/entity/RepetitionTask.ts
var _brand = Symbol();
var _RepetitionTask = class _RepetitionTask extends import_owlelia.Entity {
  static of(props) {
  ...
  includesDay(date, holidays) {
    if (isHoliday(date, holidays)) {
      if (!this._props.repetition.dayOfWeekHoliday.includes(date.date.getDay()) && !this._props.repetition.dayOfWeekHoliday.includes(
        date.date.getDay() + 10 * date.nthDayOfWeek
      )) {
        return false;
      }
    } else {
      if (!this._props.repetition.dayOfWeek.includes(date.date.getDay()) && !this._props.repetition.dayOfWeek.includes(
        date.date.getDay() + 10 * date.nthDayOfWeek
      )) {
        return false;
      }
...
var _brand2 = Symbol();
var _Repetition = class _Repetition extends import_owlelia2.ValueObject {
  static get everyDay() {
...
var TaskRepositoryImpl = class {
  ...
  loadRepetitionTasks() {
    return (0, import_owlelia3.fromPromise)(
      this.appHelper.loadFile(this.repetitionTasksFilePath).then(
        (tasksStr) => tasksStr.split("\n").filter((line) => !line.startsWith("//") && line.trim() !== "").map((line) => line.split(",")).filter((cols) => cols.length > 1).map(([name, repetition, baseDate]) => {
          var _a, _b;
  ...
  loadHolidays() {
    return (0, import_owlelia3.fromPromise)(
      this.holidaysFilePath ? this.appHelper.loadFile(this.holidaysFilePath).then(
        (holidaysStr) => holidaysStr.split("\n").filter((line) => !line.startsWith("//") && line.trim() !== "").map(import_owlelia3.DateTime.of).filter((x) => !Number.isNaN(x.date.getTime()))
        // TODO: owleliaに実装する
      ) : Promise.resolve([])
    );
  }
...
var _brand3 = Symbol();
var _Timer = class _Timer extends import_owlelia4.ValueObject {
  static of(props) {
...
var TimerRepositoryImpl = class {
  ...
  loadTimer() {
    return (0, import_owlelia5.fromPromise)(
...
var TaskServiceImpl = class {
  ...
  async insertTasksToDailyNote(date) {
    ...
    this.appHelper.insertStringToActiveFile(
      tasks.filter((x) => x.shouldTry(date, holidays)).map((x) => `${x.indent}- [ ] ${x.name}`).join("\n")
    );
  }
  ...
  calcDatesInFuture(task2, holidays) {
    const today2 = import_owlelia6.DateTime.today();
    return today2.toDate(today2.plusMonths(3)).filter((x) => task2.shouldTry(x, holidays));
  }
};
...
function createEvents(input) {
  return input.map((event) => {
    ...
    return {
      ...
      allDay: (_a = event.allDay) != null ? _a : noTimePart(event.start) && noTimePart(event.end),
...
function instance$4($$self, $$props, $$invalidate) {
  ...
  $$self.$$set = ($$props2) => {
    if ("chunk" in $$props2)
      $$invalidate(34, chunk = $$props2.chunk);
    if ("longChunks" in $$props2)
      $$invalidate(35, longChunks = $$props2.longChunks);
    if ("inPopup" in $$props2)
      $$invalidate(36, inPopup = $$props2.inPopup);
  };
  $$self.$$.update = () => {
...
function create_fragment$22(ctx) {
  ...
  return {
    c() {
      ...
      attr(div2, "class", div2_class_value = /*$theme*/
      ctx[12].day + " " + /*$theme*/
      ((_a = ctx[12].weekdays) == null ? void 0 : _a[
        /*date*/
        ctx[0].getUTCDay()
      ]) + /*isToday*/
      (ctx[7] ? " " + /*$theme*/
...
function instance$22($$self, $$props, $$invalidate) {
  ...
  $$self.$$set = ($$props2) => {
    ...
    if ("chunks" in $$props2)
      $$invalidate(27, chunks = $$props2.chunks);
    if ("longChunks" in $$props2)
...
function create_fragment3(ctx) {
  ...
  return {
    ...
    p(ctx2, [dirty]) {
      ...
      if (dirty & /*options*/
      4)
        calendar_changes.options = /*options*/
        ctx2[2];
...
function instance3($$self, $$props, $$invalidate) {
  ...
  let { holidays } = $$props;
  let currentTask;
  const handleClickTask = (task2) => __awaiter(void 0, void 0, void 0, function* () {
      ...
      case "recording":
        ...
        this.appHelper.replaceStringInActiveLine(
          lineTimeStatus.getNextStatusLine(
            line,
            (await this.getTimer()).stop(import_owlelia8.DateTime.now())
          )
        );
  ...
  return [
    {
      ...
      checkCallback: (checking) => {
        if (appHelper.getActiveFile() && appHelper.getActiveMarkdownView()) {
          if (!checking) {
            const date = import_owlelia9.DateTime.from(
        ...
        if (appHelper.getActiveFile() && appHelper.getActiveMarkdownView()) {
          if (!checking) {
            ...
            appHelper.replaceStringInActiveLine(
              `${prefix}${import_owlelia9.DateTime.now().format("HH:mm")} ${content}`,
              { cursor: "last" }
            );
          }
          return true;
...
var SilhouettePlugin = class extends import_obsidian7.Plugin {
  ...
  addStatusBar() {
    ...
    this.timerService.setOnTimerHandler((timer) => {
      const timerText = timer ? toDisplayFooter(timer.getPastSeconds(import_owlelia10.DateTime.now())) : "\u672A\u6E2C\u5B9A";
  ...
  async reset() {
    ...
    this.timerService.terminate();
    (_a = this.timerStatusBar) == null ? void 0 : _a.remove();
    if (this.settings.showTimeOnStatusBar) {
      this.addStatusBar();
    }
  }
